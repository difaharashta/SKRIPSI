...
if (isset($_POST['koordinat'])) {
	...
	foreach ($koordinat as $idform => $koordinatForm) {
		mysql_query("UPDATE `diagrams` SET `x`='".$koordinatForm->x."', `y`='".$koordinatForm->y."' WHERE `id_form` = '$idform' AND `id_diagram`='$id';");
	}
}
...
window.onload = function() {
	...
	for ($i=0; $i < count($dataParentDiagram); $i++) {
		...
		//jika x dan y = 0, yang artinya posisi x dan y belum ditentukan(posisi default dari node) maka existUnmodified=true.
		...
		var render= function(r, n) {
			...
			//The following statement creates an object and assigns it to the variable forms
			//properties dari forms berupa: x,y dengan value (x,y) juga tapi x dan y dari diagrams
			//.x & .y properti yang dibuat di dalam forms
			// let's create an object named myCar and give it properties named make, model, and year as follows: mycar.make 
...
console.log("box:", set);
	...
	resetKoordinat = function(){
		...
		//method ini hanya bisa untuk reset koordinat parents saja, tapi anak2nya tidak bisa di reset koordinat
		...
		function DFSCalc() {
				...
				if (visited[node]!=true) {
					...
					if (edges[node]!=null) {
						for (var i = 0; i < edges[node].length; i++) {
							...
							stack.push(edges[node][i]); //basic dfs
						}
					}
			...
			//ambil max availSlot jadiin height
				...
				if (visited[node]!=true) {
					...
					if (edges[node]!=null) {
						for (var i = 0; i < edges[node].length; i++) {
							...
							if (origin[edges[node][i]]==null) origin[edges[node][i]] = node; //untuk memasangkan siapa ditunjuk siapa
							//push anak2 ke stack
